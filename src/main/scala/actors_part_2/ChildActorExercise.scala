...
object ChildActorExercise {
   ...
   *
   *
   * Scheme for scheduling tasks to children is round robin
   * [1-10]
   * task 1 - child 1
   * task 2 - child 2
   * .....
    ...
    def apply(): Behavior[MasterProtocol] = Behaviors.receive { (context, message) =>
      message match {
        case Initialize(nChildren) =>
          context.log.info(s"[master] initialising with $nChildren children")
          val childRefs = for {
            i <- 1 to nChildren
    ...
    def apply(masterRef: ActorRef[MasterProtocol]): Behavior[WorkerProtocol] = Behaviors.receive { (context, message) =>
      message match {
        case WorkerTask(id, text) =>
          ...
          // send back the result back to the master
