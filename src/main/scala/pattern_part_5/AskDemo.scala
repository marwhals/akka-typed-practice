...
object AskDemo {
  ...
  def askSimple(): Unit = {
    ...
    //                                           ^ temporary actor     ^ message that gets sent to the worker == the same as the user guardian
  ...
  def askFromWithinAnotherActor(): Unit = {
    val userGuardian = Behaviors.setup[WorkProtocol] { context =>
      ...
      // 0 - define extra message that I should handle as a result of ask
      ...
      context.ask(worker, ref => ComputationalTask("This ask pattern seems quite complicated", ref)) {
        // Try[WorkProtocol] => WorkProtocol message that will be sent to the actor who is calling the worker protocol
