...
object StashDemo {
  ...
  object ResourceActor {
    def apply(): Behavior[Command] = closed("42") // resource starts as closed with some initial data
      ...
      Behaviors.receive { (context, message) =>
        message match {
          ...
          case _ =>
            ...
            buffer.stash(message) // bufer is mutable. Important not to send this to other actors or futures
            Behaviors.same
        }
    ...
    def open(data: String): Behavior[Command] = Behaviors.receive { (context, message) =>
      message match {
        case Read =>
          context.log.info(s"I have read $data") // in real life this is where you would fetch some actual data
          Behaviors.same
        case Write(newData) =>
