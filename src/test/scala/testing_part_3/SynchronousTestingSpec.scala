package testing_part_3

import actors_part_2.ChildActorExercise.{Initialize, Reply, UserProtocol, WordCountReply, WordCountTask, WordCounterMaster, WorkerProtocol}
...
class SynchronousTestingSpec extends ScalaTestWithActorTestKit with AnyWordSpecLike {
  ...
  "A word counter master" should {
    ...
    "send a task to a child" in {
      ...
      val mailbox = TestInbox[UserProtocol]() // the "requesters"'s inbox
      // start processing
    ...
    "log messages" in {
      val master = BehaviorTestKit(WordCounterMaster())
      master.run(Initialize(1))
      master.logEntries() shouldBe Seq(CapturedLogEvent(Level.INFO, "[master] initialising with 1 children"))
    }
  }
}
