...
class TimeAssertionsSpec extends ScalaTestWithActorTestKit with AnyWordSpecLike {
  ...
  "A worker actor" should {
    ...
    "reply with multiple result in a timely manner" in {
...
object TimeAssertionsSpec {
  ...
  object WorkerActor {
    def apply(): Behavior[Message] = Behaviors.receiveMessage {
      case Work(replyTo) =>
        // wait a bit - simulate a long competition
        Thread.sleep(500)
